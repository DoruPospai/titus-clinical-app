...
class TitusEngine:
    ...
    def __init__(self) -> None:
        ...
        # Harti nume elemente pentru explain
        self.symp_map: dict[int, str] = {}
        self.sign_map: dict[int, str] = {}
        symp_path = DATA_CLEAN / "Symptomes.xlsx"
        sign_path = DATA_CLEAN / "Signe.xlsx"
        if symp_path.exists():
            sdf = pd.read_excel(symp_path)
            if "CodeSymptome" in sdf.columns and "NomSymptome" in sdf.columns:
                self.symp_map = dict(zip(
                    sdf["CodeSymptome"].astype(int),
                    sdf["NomSymptome"].astype(str)
                ))
        if sign_path.exists():
            sgf = pd.read_excel(sign_path)
            if "CodeSigne" in sgf.columns and "NomSigne" in sgf.columns:
                self.sign_map = dict(zip(
                    sgf["CodeSigne"].astype(int),
                    sgf["NomSigne"].astype(str)
                ))
        # Harti nume pentru explain
        self.symp_map: dict[int, str] = {}
        self.sign_map: dict[int, str] = {}
    ...
    def print_explain(self, expl: dict) -> None:
        ...
        def ename(key):
            code, nature = key
            nm = (self.symp_map.get(code) if nature == "Sympt"
                  else self.sign_map.get(code))
